(function(){
  try {
    ...
    gsap.from('.media-frame', { y: 30, opacity: 0, duration: .9, delay: .2, ease: 'power3.out' });
    gsap.utils.toArray('.bento-item, .compare-card').forEach(function(el){
      gsap.from(el, { scrollTrigger: { trigger: el, start: 'top 80%' }, y: 24, opacity: 0, duration: .6, ease: 'power3.out' });
    });
